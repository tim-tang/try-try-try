...
get(IdxNode, StatName) ->
  riak_core_vnode_master:sync_command(IdxNode, {get, StatName}, ?MASTER).

set(IdxNode, StatName, Val) ->
  riak_core_vnode_master:sync_command(IdxNode, {set, StatName, Val}, ?MASTER).

incr(IdxNode, StatName) ->
  riak_core_vnode_master:sync_command(IdxNode, {incr, StatName}, ?MASTER).

incrby(IdxNode, StatName, Val) ->
  riak_core_vnode_master:sync_command(IdxNode, {incrby, StatName, Val}, ?MASTER).

append(IdxNode, StatName, Val) ->
  riak_core_vnode_master:sync_command(IdxNode, {append, StatName, Val}, ?MASTER).

sadd(IdxNode, StatName, Val) ->
  riak_core_vnode_master:sync_command(IdxNode, {sadd, StatName, Val}, ?MASTER).
