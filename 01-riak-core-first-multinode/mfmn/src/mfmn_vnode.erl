...
handle_command({set, {ReqID, Coordinator}, Key, Value}, _Sender,
	...
	error ->
	    Val0 = statebox:new(fun mfmn_entity_state:new/0),
	    Val1 = statebox:modify({fun mfmn_entity_state:set/2, [Value]}, Val0),
	    VC0 = vclock:fresh(),
	    VC = vclock:increment(Coordinator, VC0),
	    Obj = #mfmn_obj{val=Val1, vclock=VC},
	    Keys1 = dict:store(Key, Obj, Keys0),
	    {reply, {ok, ReqID}, State#state{keys=Keys1}};
	{ok, #mfmn_obj{val=Val0} = O} ->
	    Val1 = statebox:modify({fun mfmn_entity_state:set/2, [Value]}, Val0),
	    Val2 = statebox:expire(?STATEBOX_EXPIRE, Val1),
	    Obj = mfmn_obj:update(Val2, Coordinator, O),
	    Keys1 = dict:store(Key, Obj, Keys0),
	    {reply, {ok, ReqID}, State#state{keys=Keys1}}
    end;
