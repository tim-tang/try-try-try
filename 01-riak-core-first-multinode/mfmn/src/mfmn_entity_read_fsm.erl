...
start(VNodeInfo, Op, User, Val) ->
    ReqID = mk_reqid(),
    mfmn_entity_read_fsm_sup:start_read_fsm(
...
-spec merge([vnode_reply()]) -> mfmn_obj() | not_found.
merge(Replies) ->
    Objs = [Obj || {_,Obj} <- Replies],
    mfmn_obj:merge(mfmn_entity_read_fsm, Objs).
...
%% @pure
different(A) -> fun(B) -> not mfmn_obj:equal(A,B) end.

%% @impure
%%
%% @doc Repair any vnodes that do not have the correct object.
-spec repair(atom(), string(), mfmn_obj(), [vnode_reply()]) -> io.
repair(_, _, _, []) -> io;

repair(VNode, StatName, MObj, [{IdxNode,Obj}|T]) ->
    case mfmn_obj:equal(MObj, Obj) of
